...
\For{all words $w_t \in{[1,N_m]}$ in document $m$}{
 	sample topic index $z_{m,t}=k\sim{Mult(\frac{1}{K})}$\\
	 increment document-topic count: $n_{mk}+1$\\
	 increment document-topic sum: $n_m+1$\\
	 increment topic-term count: $q_{kw_t}+1$\\
...
\While{not converged}{
	\For{all documents  $m\in{[1,M]}$}{
		\For{all words $w_t \in{[1,N_m]}$ in document $m$}{
decrement counts and sums: $n_{mk}-1,n_m-1,q_{kw_t}-1,q_k-1$\\
sample topic index $\overline{k}\sim{p(z_i|,Z_{-i},W)}$\\
increment counts and sums: $n_{mk}+1,n_m+1,q_{kw_t}+1,q_k+1$\\
	}
		}
