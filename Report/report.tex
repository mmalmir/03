...
\end{equation}
where $\theta_{m,k}$ is the topic distribution for document m and topic k. If the document distribution has higher probability for only some topics, its associated harmonic mean value would be lower. So, the model could have better ability to separate documents into different topics.
...
One of the challenges when doing unsupervised training for model, is to determine the stop criteria. This is the point where the model starts to over-fit the training data. 
